...
and if the image was on my computer rather than online, I could include it by using the same syntax with a relative path rather than website URL. 
...
Note that setting echo to false includes the output of the code, but does not include the code. If we wanted to display the data in a more elegant way, we could use the `gt` package:
...
There are lots of beautiful Western red cedars in Portland! 
